# app/application/use_cases/password_reset_use_cases.py
import hashlib
import secrets
...
class PasswordResetUseCases:
    async def request_reset(
        ...
        if not rate_limiter.allow(
            key,
            settings.password_reset_rate_max_per_key,
        ...
        if user:
            ...
            # Mail HTML simple (template possible)
            html = (
                f"<p>Hi,</p>"
                f"<p>Click the link below to reset your password "
                f"(valid {settings.password_reset_token_expire_minutes} minutes):</p>"
                f"<p><a href='{reset_link}'>Reset Password</a></p>"
                f"<p>If you didn't request this, ignore this email.</p>"
            )
            try:
                await email_sender.send_html(
                    to=email, subject="Reset your password", html=html
                )
            except Exception:
                pass
